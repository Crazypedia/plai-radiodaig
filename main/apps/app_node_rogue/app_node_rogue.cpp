...
void AppNodeRogue::_render_traffic_tab()
{
    ...
    for (int i = 0; i < max_rows && (_data.scroll_offset + i) < total; i++)
    {
        ...
        else if (is_rogue) { canvas->setTextColor(TFT_RED); canvas->drawString("!", 2, y + 1); }
    ...
    char air[24];
    if (sel.has_eirp && _is_overpowered(sel))
    {
        // For over-powered nodes, surface the estimated lower-bound EIRP here.
        snprintf(air, sizeof(air), "EIRP>=%ddBm", (int)(sel.eirp_min_dbm + 0.5f));
        canvas->setTextColor(TFT_MAGENTA);
    }
    else
    {
